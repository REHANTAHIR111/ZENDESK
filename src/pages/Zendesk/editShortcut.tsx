...
export default function addShortcut() {
  ...
  const handleOptionSelect = (option: string) => {
    ...
    useEffect(() => {
        dispatch(setPageTitle('Edit shortcut')); 
      }, []);
        ...
        <div className='gap-8 w-full mt-4 flex text-gray-600 font-normal text-[12.5px] leading-4'>
            <div className='w-[48.5%]'>
                        ...
                        {inputFields.map((field, index) => (
                            <div className='flex gap-2' key={index}>
                                ...
                                {inputFields.length > 1 ? <button className={`px-[0.4rem] mt-1 py-[0.3rem] rounded-full border hover:bg-[#E9EBED]`} onClick={() => handleRemoveField(index)} ><IconMinus className='w-4 h-4'/></button> : ''}
                                    ...
                                    <button className={`px-[0.4rem] mt-1 py-[0.3rem] rounded-full border hover:bg-[#E9EBED]`} disabled={inputFields?.length >= 4} onClick={handleAddField}>
                ...
                <div className='flex mt-4 flex gap-3'>
                    ...
                    <button className='py-1.5 px-3 border border-gray-300 hover:bg-gray-200 font-bold rounded' type='button' onClick={() => {localStorage.setItem('header', 'Shortcuts'), navigate('/zendesk/shortcuts')}}>Cancel</button>
                </div>
            </div>
            <div className='w-44'>
