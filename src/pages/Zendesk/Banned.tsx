...
const Banned = (props: any) => {
    ...
    const { effectiveColumns, resetColumnsOrder } = useDataTableColumns<any>({
        ...
        columns: [
            {
                accessor: 'Visitor/Ip address',
                title: 'Visitor/Ip address',
                draggable: true,
                sortable: false,
                width: 150,
            ...
            <div className="">
                <div className="flex mb-3 w-full justify-between">
                    <div className='gap-3 flex'>
                        ...
                        <select id="countries" className="form-select rounded w-[8.5rem] h-[2rem] py-0">
                            <option selected>Filter</option>
                            <option>Ip address</option>
                            <option>Visitors</option>
                        </select>
                        <button className='btn btn-sm shadow-none hover:bg-[#5293C7] rounded text-white h-8 bg-[#3681BE]' onClick={() => {localStorage.setItem('header','Add banned IP address'), navigate('/zendesk/banned/addvisitor')}}>Add visitor</button>
                    </div>
                    <div className='flex gap-3'>
                        <label className='text-sm pt-1 w-44 text-end font-normal'>1065 entries</label>
                    </div>
                </div>
