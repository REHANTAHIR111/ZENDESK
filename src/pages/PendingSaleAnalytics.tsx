...
const PendingSaleAnalytics = (props: any) => {
    ...
    useEffect(() => {
        ...
        if (localStorage.getItem("permissions") != 'undefined' && localStorage.getItem("permissions") != null) {
            ...
            if (permisssion && permisssion?.length >= 1) {
                ...
                if (permisssion?.filter((item: { module: { code: string; }; view: number; }) => item?.module?.code == "pending_analytics" && item?.view === 1).length <= 0) {
                    navigate(-1)
                }
            }
        } else {
    ...
    const getData = (start: string, end: string) => {
        // setDatePanel(false)
        setloader(true)
        get('pending-sales-report/' + start + '_' + end).then((responseJson: any) => {
    ...
    const getSalesReport = () => {
        ...
        if (type?.value == 1) {
            fetchData('pending-report-export-product', data).then((responseJson: any) => {
                const url = window.URL.createObjectURL(responseJson);
                const a = document.createElement('a');
                a.style.display = 'none';
                a.href = url;
                // the filename you want
                a.download = 'pending-report-product.csv';
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
        ...
        if (type?.value == 2) {
            fetchData('pending-report-export-brand', data).then((responseJson: any) => {
                const url = window.URL.createObjectURL(responseJson);
                const a = document.createElement('a');
                a.style.display = 'none';
                a.href = url;
                // the filename you want
                a.download = 'pending-report-brand.csv';
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
        ...
        if (type?.value == 3) {
            fetchData('pending-report-export-category', data).then((responseJson: any) => {
                const url = window.URL.createObjectURL(responseJson);
                const a = document.createElement('a');
                a.style.display = 'none';
                a.href = url;
                // the filename you want
                a.download = 'pending-report-category.csv';
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
        ...
        if (type?.value == 4) {
            fetchData('pending-report-export-pmethod', data).then((responseJson: any) => {
                const url = window.URL.createObjectURL(responseJson);
                const a = document.createElement('a');
                a.style.display = 'none';
                a.href = url;
                // the filename you want
                a.download = 'pending-report-payment-method.csv';
                document.body.appendChild(a);
                a.click();
                window.URL.revokeObjectURL(url);
    ...
    const simpleColumnStacked: any = {
        series: [
            {
                name: t('totalNumberOfPendingOrders'),
                data: totalOrderData,
            ...
            {loader ?
                ...
                <>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-3 text-white mt-3">
                        ...
                        {/* Sessions */}
                        <Link
                            to={'/sales/order'} className="panel bg-gradient-to-r from-violet-500 to-violet-400">
                            <div className="ltr:mr-1 rtl:ml-1 text-md font-semibold">{t('totalPendingValue')}</div>
                            <div className="text-3xl font-bold ltr:mr-3 rtl:ml-3 mt-5">{t('sr')}&nbsp;{totalAmountRecievedBox?.toLocaleString('EN-US')}</div>
                            ...
                            <div className="flex items-center font-semibold">
                                <IconEye className="ltr:mr-2 rtl:ml-2 shrink-0" />
                                {t('averagePendingValue')}: {StatusData?.filter((item: { status: number; }) => item?.status == 8)[0]?.totalavg?.toLocaleString('EN-US')}
                            </div>
                        </Link>
                        <Link to={'/sales/order'}
                            className="panel bg-gradient-to-r from-slate-500 to-slate-400">
                            <div className="ltr:mr-1 rtl:ml-1 text-md font-semibold">{t('totalOrderPending')}  </div>
                        ...
                        <div className="grid gap-3 mb-6">
                            <div className="panel h-full xl:col-span-2">
                                <p className="text-lg dark:text-white-light/90">
                                    {t('totalPending')} <span className="text-primary ml-2">{t('sr')} {OrderPrice?.toLocaleString('EN-US')}</span>
                                </p>
                                <div className="relative">
                                    <div className="bg-white dark:bg-black rounded-lg overflow-hidden">
                                        {loading ? (
...
export default PendingSaleAnalytics;